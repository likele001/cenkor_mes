...
from app.services.feishu.settings import get_feishu_settings_raw, get_group_chat_id
...
def _dept_auto_chat_id(db: Session, department_id: int | None, cfg: dict) -> str:
    if not department_id:
        return ""
    dept = db.get(Department, department_id)
    if not dept or not dept.feishu_chat_group_code:
        return ""
    return get_group_chat_id(cfg, dept.feishu_chat_group_code)
...
def resolve_targets(
    ...
    *,
    ...
    """返回 [{kind: user|chat, ref: open_id|chat_id, user_id?, chat_code?}]"""
    ...
    def add_user(uid: int) -> None:
        ...
        if key in seen:
            return
        seen.add(key)
        out.append({"kind": "user", "ref": oid, "user_id": uid})

    def add_chat(chat_id: str, chat_code: str = "") -> None:
        ...
        if key in seen:
            return
        seen.add(key)
        out.append({"kind": "chat", "ref": cid, "chat_code": chat_code})
    ...
    for code in target_codes:
        if code == "assigned_employee":
            ...
            if gcode == "dept_auto":
                add_chat(_dept_auto_chat_id(db, department_id, cfg), "dept_auto")
            else:
                add_chat(get_group_chat_id(cfg, gcode), gcode)
